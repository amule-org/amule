                       ...
                       std::string &fs_out)
{
	...
	if (!_fullpath(root_real, root.c_str(), PATH_MAX)) return false;
	const std::string joined = std::string(root_real) + "\\" + rel;
	if (!_fullpath(fs_real, joined.c_str(), PATH_MAX)) return false;
	struct stat st;
	if (::stat(fs_real, &st) != 0) return false;
#else
	if (!realpath(root.c_str(), root_real)) return false;
	const std::string joined = std::string(root_real) + "/" + rel;
	if (!realpath(joined.c_str(), fs_real)) return false;
#endif
	...
	while (root_len > 1
	       && (root_real[root_len - 1] == '/'
	        || root_real[root_len - 1] == '\\')) {
		root_real[--root_len] = '\0';
	}
	if (std::strncmp(fs_real, root_real, root_len) != 0) return false;
	const char sep = fs_real[root_len];
	if (sep != '/' && sep != '\\' && sep != '\0') return false;
