...
`HEAD` returns the stream's headers and no body. Any other method is `405` with `Allow: GET, HEAD`, like every other route. A trailing slash is stripped first, so `/api/v1/events/` opens the same stream.
